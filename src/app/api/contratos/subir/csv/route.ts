...
export async function POST(request: any) {
  ...
  try {
    ...
    if (!file) {
      ...
      return NextResponse.json(
        {
          message: "Formato de archivo no válido. Se requiere un archivo CSV.",
        },
        {
          status: 400,
        }
    ...
    const result = await parse(redFile, {
      columns: true,
      delimiter: ";",
      cast: (value, context) => {
        if (context.column === "fecha_contrato") {
            const transformedDate = transformDate(value);
            console.log(transformedDate);
          return transformedDate;
        }
        if (context.column === "fecha_instalacion") {
          const transformedDate = transformDate(value);
          console.log(transformedDate);
          return transformedDate;
        }
    ...
    if (queryres == 1) {
      ...
      return NextResponse.json(
        {
          message:
            "Error de Sintaxis en el archivo CSV. Por favor, verifique la estructura del archivo.",
        },
        {
          status: 400,
        }
